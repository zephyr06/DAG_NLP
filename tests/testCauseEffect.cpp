...
TEST(RTDA, v3) {
    ...
    initialEstimate << 0, 100, 200, 321, 415, 500, 0, 100, 201, 320, 415, 500,
        1, 309, 416, 202, 300, 1, 322;
    SFOrder jobOrder(tasksInfo, initialEstimate);
    std::vector<uint> processorJobVec;
    VectorDynamic expectStv = SimpleOrderScheduler::schedule(
        dagTasks, tasksInfo, scheduleOptions, jobOrder, processorJobVec);
    EXPECT(assert_equal(expectStv, initialEstimate));

    std::vector<int> causeEffectChain = {3, 2, 1, 0};
    auto res =
        GetRTDAFromSingleJob(tasksInfo, causeEffectChain, initialEstimate);
    RTDA resM = GetMaxRTDA(res);
    EXPECT_LONGS_EQUAL(120, resM.reactionTime);
    EXPECT_LONGS_EQUAL(501, resM.dataAge);  // T3_1 -> T0_2 + 600

    EXPECT(ExamAll_Feasibility(dagTasks, tasksInfo, initialEstimate,
