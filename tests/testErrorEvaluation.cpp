...
TEST(IO, ReadWriteResult) {
    ...
    WriteToResultFile(pathDataset, file, res, TOM, "ReactionTimeObj");
    OrderOptDAG_SPACE::ScheduleResult res2 =
        ReadFromResultFile(pathDataset, file, TOM, "ReactionTimeObj");
...
TEST(ExamDBF_Feasibility, v1) {
    ...
    PrintSchedule(tasksInfo, initial);
    EXPECT(ExamDBF_Feasibility(dagTasks, tasksInfo, initial, processorJobVec,
                               processorNum));
    EXPECT(!ExamDBF_Feasibility(dagTasks, tasksInfo, initial, processorJobVec,
                                processorNum - 1));
    EXPECT(!ExamDBF_Feasibility(dagTasks, tasksInfo, initial, processorJobVec,
...
TEST(sensorFusion, v1) {
    ...
    OrderOptDAG_SPACE::DAG_Model dagTasks = ReadDAG_Tasks(
        GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n5_v9.csv",
        "orig");
    TaskSet tasks = dagTasks.tasks;
    TaskSetInfoDerived tasksInfo(tasks);
    ...
    initial << 3, 5, 1, 6, 7;
    EXPECT_LONGS_EQUAL(
        3, ObtainSensorFusionError(dagTasks, tasksInfo, initial)(0));
}
...
TEST(sensorFusion, multi_rate) {
    ...
    OrderOptDAG_SPACE::DAG_Model dagTasks = ReadDAG_Tasks(
        GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n5_v17.csv",
        "orig");
    TaskSet tasks = dagTasks.tasks;
    TaskSetInfoDerived tasksInfo(tasks);
    ...
    EXPECT_LONGS_EQUAL(
        100, ObtainSensorFusionError(dagTasks, tasksInfo, initial)(0));
    // 1 finishes 5 + 11 = 16, 3 at 2+13, 4 at 0+14
    EXPECT_LONGS_EQUAL(
        2, ObtainSensorFusionError(dagTasks, tasksInfo, initial)(1));

    // 1 depends on 2,3,4;
    // 2 finishes at 104+12, 3 at 2+13, 4 at 101+14
    EXPECT_LONGS_EQUAL(
        101, ObtainSensorFusionError(dagTasks, tasksInfo, initial)(2));

    initial << 16, 107, 5, 3, 104, 2, 0,
        101;  // 2 finishes at 104+12, 3 at 2+13, 4 at 101+14
    // 1 finishes 5 + 11 = 16, 3 at 2+13, 4 at 0+14
    EXPECT_LONGS_EQUAL(
        2, ObtainSensorFusionError(dagTasks, tasksInfo, initial)(0));
    EXPECT_LONGS_EQUAL(
        2, ObtainSensorFusionError(dagTasks, tasksInfo, initial)(1));
...
//     "TaskData/test_n5_v1.csv", "orig", 2); // single-rate dag TaskSet tasks =
//     dagTasks.tasks; TaskSetInfoDerived tasksInfo(tasks);
...
//     // About expect: there are two chains, each chain has a max data age and
//     a max reaction time EXPECT_LONGS_EQUAL(364 * 2 + 375 * 2, res.obj_);
// }
...
//     weightInMpRTDA = 0.5;
//     DAG_Model dagTasks = ReadDAG_Tasks(GlobalVariablesDAGOpt::PROJECT_PATH +
//     "TaskData/test_n3_v18.csv", "orig", 1); // single-rate dag
//     doScheduleOptimization = 1;
//     ScheduleResult res = ScheduleDAGModel<LSchedulingFreeTA>(dagTasks,
//     processorNum); EXPECT_LONGS_EQUAL(5 + 4, res.obj_);
// }
