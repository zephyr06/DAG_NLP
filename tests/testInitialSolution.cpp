...
TEST(RunQueue, V1) {
    ...
    OrderOptDAG_SPACE::DAG_Model dagTasks = ReadDAG_Tasks(
        GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n5_v10.csv", "RM");
    ...
    q.insert({0, 0});
    q.insert({1, 0});
    AssertEqualScalar(1, q.front());  // both 0 and 1 is okay
    q.insert({5, 0});
...
TEST(toplogical, sortv1) {
    using namespace OrderOptDAG_SPACE;
    OrderOptDAG_SPACE::DAG_Model dagTasks = ReadDAG_Tasks(
        GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n5_v40.csv",
        "orig");
    ...
    std::vector<int> topoOrder =
        TopologicalSortSingle(originTasks, dagTasks, graphBoost, indexesBGL);
...
TEST(FindSinkTaskIds, v2) {
    using namespace OrderOptDAG_SPACE;
    OrderOptDAG_SPACE::DAG_Model dagTasks = ReadDAG_Tasks(
        GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n5_v66.csv",
        "orig");
    auto chains = dagTasks.FindSinkTaskIds();
...
//     "TaskData/test_n5_v14.csv", "orig"); TaskSet tasks = dagTasks.tasks;
//     TaskSetInfoDerived tasksInfo(tasks);
//     VectorDynamic inital = GenerateInitial_Custom_DAG(dagTasks,
//     tasksInfo.sizeOfVariables, tasksInfo.variableDimension, 0);
...
//     OrderOptDAG_SPACE::DAG_Model dagTasks =
//     ReadDAG_Tasks(GlobalVariablesDAGOpt::PROJECT_PATH +
//     "TaskData/test_n5_v40.csv", "orig"); TaskSet tasks = dagTasks.tasks;
//     TaskSetInfoDerived tasksInfo(tasks);
//     VectorDynamic inital = GenerateInitial_Custom_DAG(dagTasks,
//     tasksInfo.sizeOfVariables, tasksInfo.variableDimension, 0);
//     // for (uint i = 0; i < dagTasks.tasks.size(); i++)
...
//     OrderOptDAG_SPACE::DAG_Model dagTasks =
//     ReadDAG_Tasks(GlobalVariablesDAGOpt::PROJECT_PATH +
//     "TaskData/test_n3_v4.csv", "orig"); TaskSet tasks = dagTasks.tasks;
//     TaskSetInfoDerived tasksInfo(tasks);
//     VectorDynamic initial = GenerateInitial_Custom_DAG(dagTasks,
//     tasksInfo.sizeOfVariables, tasksInfo.variableDimension, 0);
...
//     OrderOptDAG_SPACE::DAG_Model dagTasks =
//     ReadDAG_Tasks(GlobalVariablesDAGOpt::PROJECT_PATH +
//     "TaskData/test_n6_v1.csv", "orig"); TaskSet tasks = dagTasks.tasks;
//     TaskSetInfoDerived tasksInfo(tasks);
//     VectorDynamic initial = GenerateInitial_Custom_DAG(dagTasks,
//     tasksInfo.sizeOfVariables, tasksInfo.variableDimension, 0); std::cout <<
//     initial << std::endl; VectorDynamic expected = initial; expected << 0,
//     45, 68, 5, 50, 60, 73, 85, 80, 75; assert_equal(expected, initial);
// }
...
TEST(PrintSchedule, SortJobSchedule) {
    ...
    std::vector<std::pair<std::pair<double, double>, JobCEC>> timeJobVector =
        ObtainAllJobSchedule(tasksInfo, initial);
