...
TEST(SFOrder, constructor_v3) {
    OrderOptDAG_SPACE::DAG_Model dagTasks = ReadDAG_Tasks(
        GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n3_v24.csv",
        "orig");
    ...
    initialSTV << 440, 0, 3560, 5560, 7560, 9560, 1047, 3047, 5047, 7047, 9047;
...
TEST(DBF_error, v1) {
    ...
    PrintSchedule(tasksInfo, initial);
    EXPECT_LONGS_EQUAL(0, DBF_Error(dagTasks, tasksInfo, initial,
                                    processorJobVec, processorNum));
    initial << 0, 0, 0;
    EXPECT_LONGS_EQUAL(4, DBF_Error(dagTasks, tasksInfo, initial,
                                    processorJobVec, processorNum));
}
TEST(sensorFusion, v1_no_fork) {
    ...
    OrderOptDAG_SPACE::DAG_Model dagTasks = ReadDAG_Tasks(
        GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n3_v10.csv",
        "orig");
    TaskSet tasks = dagTasks.tasks;
    TaskSetInfoDerived tasksInfo(tasks);
    ...
    initial << 2, 1, 0, 3, 4;
    EXPECT_LONGS_EQUAL(
        0, ObtainSensorFusionError(dagTasks, tasksInfo, initial)(0));
...
TEST(sched, v2) {
    ...
    VectorDynamic initialSTV = SFOrderScheduling(
        dagTasks.tasks, tasksInfo, processorNum, sfOrder, processorJobVec_);
    EXPECT(assert_equal(initial, initialSTV));
    ...
    EXPECT(ExamBasic_Feasibility(dagTasks, tasksInfo, initialSTV,
                                 processorJobVec_, processorNum));
}
TEST(SFOrderSchedule, test1) {
    using namespace OrderOptDAG_SPACE;
    // DAG_Model dagTasks = ReadDAG_Tasks(GlobalVariablesDAGOpt::PROJECT_PATH +
    // "TaskData/" + testDataSetName + ".csv", "orig");

    for (int taskSetId = 1; taskSetId <= 79; taskSetId++) {
        if (taskSetId == 41) {  // ignore task sets with double execution time
            taskSetId = 45;
        }
        DAG_Model dagTasks = ReadDAG_Tasks(
            GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n5_v" +
                std::to_string(taskSetId) + ".csv",
            "orig");
        ...
        SFOrder sfOrder(tasksInfo, initialSTV);
        auto startTimeVector =
            SFOrderScheduling(dagTasks.tasks, tasksInfo, 2, sfOrder);
...
TEST(WhetherStartFinishTooLong, v1) {
    ...
    EXPECT(!WhetherStartFinishTooLong(accumLengthMin, j00, 1, tasksInfo,
                                      sfOrder, startP));
    EXPECT(!WhetherStartFinishTooLong(accumLengthMin, j00, 2, tasksInfo,
                                      sfOrder, startP));
    EXPECT(WhetherStartFinishTooLong(accumLengthMin, j00, 3, tasksInfo, sfOrder,
                                     startP));
    std::cout << "After inserting finish at 3: " << std::endl;
    sfOrder.InsertFinish(j00, 3);
    sfOrder.print();
    EXPECT(WhetherStartFinishTooLong(accumLengthMin, j00, 4, tasksInfo, sfOrder,
                                     startP));
}
...
// {
//     DAG_Model dagTasks = ReadDAG_Tasks(GlobalVariablesDAGOpt::PROJECT_PATH +
//     "TaskData/test_n3_v18.csv", "orig"); TaskSet tasks = dagTasks.tasks;
//     TaskSetInfoDerived tasksInfo(tasks);

//     int processorNum = 2;
//     VectorDynamic initial = ListSchedulingLFTPA(dagTasks, tasksInfo,
//     processorNum); initial << 0, 10, 1, 1; SFOrder sfOrder(tasksInfo,
//     initial);
...
//     IterationStatus<SimpleOrderScheduler> status(dagTasks, tasksInfo,
//     sfOrder, scheduleOptions); EXPECT_LONGS_EQUAL(1,
//     FindLongestChainJobIndex<SimpleOrderScheduler>(status)[0]); initial << 0,
//     10, 1, 11; SFOrder sfOrder2(tasksInfo, initial);
//     IterationStatus<SimpleOrderScheduler> status2(dagTasks, tasksInfo,
//     sfOrder2, scheduleOptions); EXPECT_LONGS_EQUAL(0,
//     FindLongestChainJobIndex<SimpleOrderScheduler>(status2)[0]);
// }
...
TEST(RTSSIC, Wang21_DBF) {
    ...
    VectorDynamic initial = ListSchedulingLFTPA(
        dagTasks, tasksInfo, GlobalVariablesDAGOpt::coreNumberAva,
        processorIdVec);
}
...
// TODO: always considering trying to incorporate TDD more into my real coding
// work
TEST(a, b) {
    ...
    // VectorDynamic initial = ListSchedulingLFTPA(dagTasks, tasksInfo,
    // processorNum); initial << 0, 10, 1, 1; SFOrder sfOrder(tasksInfo,
    // initial); IterationStatus status(dagTasks, tasksInfo, sfOrder,
    // processorNum); VectorDynamic stv = status.startTimeVector_;
    // EXPECT(initial, stv);
}
