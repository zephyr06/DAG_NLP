...
TEST(average_performance, v1)
{
    ...
    std::vector<BatchResult> batchResVec = OrderOptDAG_SPACE::BatchOptimizeOrder<OptimizeSF::RTDAExperimentObj>(baselineMethods, dataSetFolder, 1);
    EXPECT_THAT(batchResVec[BaselineMethods::TOM_Fast].performance, testing::Le(18744.6 * 1.015)); // remove 1.015 if deciding giving up selectInitialFromPool
