   ...
   protected:
    void SetUp() override {
        dagTasks = ReadDAG_Tasks(
            GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n3_v30.csv",
            "orig", 1);
        ...
        jobOrder.print();
        VectorDynamic initial2 = SFOrderScheduling(
            dagTasks.tasks, tasksInfo, scheduleOptions.processorNum_, jobOrder,
            processorJobVec);
    };
...
class DAGScheduleOptimizerTest2 : public ::testing::Test {
   protected:
    void SetUp() override {
        dagTasks = ReadDAG_Tasks(
            GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n2_v2.csv",
            "orig", 1);
        tasks = dagTasks.tasks;
        tasksInfo = TaskSetInfoDerived(tasks);
...
TEST_F(DAGScheduleOptimizerTest2, IterateTimeInstanceSeq) {
    ...
    for (int i = 0; i < N; i++)
        jobQueueTaskSet.push_back(
            JobQueueOfATask(dagTasks.tasks[i],
                            tasksInfo.hyper_period / dagTasks.tasks[i].period));
    ...
    std::vector<TimeInstance> instanceOrderOpt{{'s', JobCEC(0, 0)},
                                               {'f', JobCEC(0, 0)},
                                               {'s', JobCEC(1, 0)},
                                               {'f', JobCEC(1, 0)}};

    for (uint j = 0; j < instanceOrderOpt.size(); j++)
        EXPECT_TRUE(instanceOrderOpt[j] ==
                    permutationStatus.orderOpt_.instanceOrder_[j]);
}

TEST_F(DAGScheduleOptimizerTest1, FindAllJobOrderPermutations) {
    PermutationStatus permSta =
        FindGlobalOptRTDA(dagTasks, tasksInfo, scheduleOptions);
    // std::vector<std::vector<TimeInstance>> instSeqAllActual =
    // FindAllJobOrderPermutations(dagTasks, tasksInfo);
