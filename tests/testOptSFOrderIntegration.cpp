...
TEST(IntegrationTest, v1rt) {
    using namespace OrderOptDAG_SPACE;
    DAG_Model dagTasks = ReadDAG_Tasks(
        GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n3_v27.csv",
        "orig");
    // chain is: 2 -> 1 -> 0
    ScheduleResult sth;
    OrderOptDAG_SPACE::OptimizeSF::ScheduleOptions scheduleOption;
...
TEST(IntegrationTest, v2rt) {
    using namespace OrderOptDAG_SPACE;
    DAG_Model dagTasks = ReadDAG_Tasks(
        GlobalVariablesDAGOpt::PROJECT_PATH + "TaskData/test_n3_v28.csv",
        "orig");
   ...
   protected:
    void SetUp() override {
        ...
        dagScheduleOptimizer =
            DAGScheduleOptimizer<SimpleOrderScheduler, ReactionTimeObj>(
...
TEST_F(DAGScheduleOptimizerTest1, UpdateStatus) {
    ...
    initial << 0, 199, 200, 300, 499, 511, 1, 198, 200, 399, 400, 599, 187, 201,
        500, 178, 490, 107, 307;
    SFOrder jobOrder(tasksInfo, initial);
    JobGroupRange uselessRange(0, 100);
    // LLint uselessFinishP = 100;

    IterationStatus<SimpleOrderScheduler, ReactionTimeObj> statusBestFound =
        dagScheduleOptimizer.statusPrev;
    SFOrder jobOrderBestFound = jobOrder;
    EXPECT_FALSE(dagScheduleOptimizer.CompareAndUpdateStatus(
        jobOrder, statusBestFound, jobOrderBestFound));
    // EXPECT_EQ(0, dagScheduleOptimizer.UpdateStatus(jobOrder, uselessRange,
    // uselessFinishP));
}
