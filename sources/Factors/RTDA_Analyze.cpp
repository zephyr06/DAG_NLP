...
std::vector<RTDA> GetRTDAFromSingleJob(const TaskSetInfoDerived &tasksInfo,
    ...
    for (LLint startInstanceIndex = 0; startInstanceIndex <= totalStartJobs;
        ...
        resVec[startInstanceIndex].reactionTime =
            ...
            GetStartTime({causeEffectChain[0], startInstanceIndex}, x,
                         tasksInfo);
        ...
        if (startInstanceIndex > 0 &&
            ...
            resVec[startInstanceIndex - 1].dataAge =
                GetStartTime(lastReaction, x, tasksInfo) +
                tasks[lastReaction.taskId].executionTime -
                GetStartTime({causeEffectChain[0], startInstanceIndex - 1}, x,
                             tasksInfo);
        }
    }
    return resVec;
...
std::vector<RTDA> GetRTDAFromSingleJob(const TaskSetInfoDerived &tasksInfo,
    ...
    for (int i = 0; i < tasksInfo.N; i++) {
        for (int j = 0; j < int(tasksInfo.sizeOfVariables[i]); j++) {
            LLint index_overall =
                IndexTran_Instance2Overall(i, j, tasksInfo.sizeOfVariables);
...
RTDA GetMaxRTDA(const TaskSetInfoDerived &tasksInfo,
    ...
    std::vector<RTDA> rtdaVec =
        GetRTDAFromSingleJob(tasksInfo, causeEffectChain, startTimeVector);
...
double ObjRT(const std::vector<RTDA> &rtdaVec) {
    double res = 0;
    for (auto &r : rtdaVec) res += r.reactionTime;
    return res;
}
...
std::unordered_map<JobCEC, std::vector<JobCEC>> GetRTDAReactChainsFromSingleJob(
    ...
    for (LLint startInstanceIndex = 0; startInstanceIndex <= totalStartJobs;
        ...
        for (uint j = 1; j < causeEffectChain.size(); j++) {
            double currentJobFT = GetFinishTime(firstJob, x, tasksInfo);
            LLint jobIndex = 0;  // TODO: test efficiency and consider whether
                                 // make this faster
            while (GetStartTime({causeEffectChain[j], jobIndex}, x, tasksInfo) <
                   currentJobFT) {
                jobIndex++;
...
// TODO: improve efficiency, more accurate jobIndex
JobCEC FindFirstReactJob(JobCEC source, int next_task_id, SFOrder &jobOrder) {
    LLint instIndexFirstJob = jobOrder.GetJobFinishInstancePosition(source);
    LLint jobIndex = 0;
    while (true) {
        JobCEC jobCurr{next_task_id, jobIndex};
    ...
    return JobCEC(next_task_id, jobIndex);
}
...
std::unordered_map<JobCEC, std::vector<JobCEC>> GetReactionChainMap(
    ...
    for (LLint startInstanceIndex = 0; startInstanceIndex <= totalStartJobs;
        ...
        for (uint j = 1; j < causeEffectChain.size(); j++) {
            firstJob =
                FindFirstReactJob(firstJob, causeEffectChain[j], jobOrder);
            react_chain.push_back(firstJob);
        }
...
JobCEC FindLastReadingJob(JobCEC sink_job, int source_task_id,
                          SFOrder &jobOrder,
                          const TaskSetInfoDerived &tasksInfo) {
    LLint instIndexFirstJob = jobOrder.GetJobStartInstancePosition(sink_job);
    LLint max_last_job_start_time = GetDeadline(sink_job, tasksInfo) -
                                    GetExecutionTime(sink_job, tasksInfo);
    int max_prev_job_index =
        max_last_job_start_time / tasksInfo.tasks[source_task_id].period +
        1;  // probably does not need +1
    LLint jobIndex = max_prev_job_index;
    while (true) {
        JobCEC jobCurr{source_task_id, jobIndex};
    ...
    return JobCEC(source_task_id, jobIndex);
}
std::unordered_map<JobCEC, std::vector<JobCEC>> GetDataAgeChainMap(
    ...
    for (LLint startInstanceIndex = 0; startInstanceIndex < totalStartJobs;
        ...
        for (int j = causeEffectChain.size() - 2; j >= 0; j--) {
            lastJob = FindLastReadingJob(lastJob, causeEffectChain[j], jobOrder,
                                         tasksInfo);
            job_chain.push_back(lastJob);
        }
