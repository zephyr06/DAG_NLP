...
inline double ComputationTime_IJK(double startTime_i, const Task &task_i, double startTime_j,
  ...
  if (startTime_i <= startTime_k &&
      startTime_k + task_k.executionTime <= startTime_j + task_j.executionTime) {
    return task_k.executionTime;
  } else
    return 0;
}
inline double ComputationTime_IJK(double startTime_i, double startTime_j, double length_j, double startTime_k,
                                  double length_k) {
...
ExtractJobsPerProcessorIC(TaskSet &tasks, std::vector<LLint> &sizeOfVariables, VectorDynamic &startTimeVector,
                          std::vector<uint> &processorJobVec, int processorNum) {
...
public:
  ...
  DBF_ConstraintFactor(Key key, TaskSet tasks, std::vector<LLint> sizeOfVariables, LLint errorDimension,
                       MAP_Index2Data mapIndex, std::vector<bool> maskForEliminate,
                       ProcessorTaskSet processorTasks, SharedNoiseModel model)
      : NoiseModelFactor1<VectorDynamic>(model, key), tasks(tasks), sizeOfVariables(sizeOfVariables),
        N(tasks.size()), errorDimension(errorDimension), maskForEliminate(maskForEliminate),
        mapIndex(mapIndex), processorTasks(processorTasks) {
    length = 0;
  ...
  gtsam::Vector evaluateError(const VectorDynamic &startTimeVector,
    ...
    if (H) {
      ...
      if (debugMode == 1) {
        std::cout << "The Jacobian matrix of DBF_ConstraintFactor is " << std::endl << *H << std::endl;
        std::cout << Color::green << "The input startTimeVector of DBF is " << startTimeVector << Color::def
                  << std::endl;
        std::cout << "The error vector of DBF is " << Color::blue << f(startTimeVector) << Color::def
                  << std::endl;
      }
      ...
      if (debugMode == 1) {
        ...
        std::cout << "The Jacobian matrix of DBF_ConstraintFactor is " << std::endl << sth << std::endl;
        std::cout << Color::green << "The input startTimeVector of DBF is " << startTimeVector << Color::def
                  << std::endl;
        std::cout << "The error vector of DBF is " << Color::blue << f(startTimeVector) << Color::def
                  << std::endl;
  ...
  MatrixDynamic JacobianAnalytic(const VectorDynamic &startTimeVectorOrig) const {
    ...
    SM_Dynamic j_map =
        JacobianElimination(length, lengthCompressed, sizeOfVariables, mapIndex, mapIndex_True2Compress);
    return j_yx * j_map;
  }
   ...
   */
  void addMappingFunctionOld(VectorDynamic &resTemp, MAP_Index2Data &mapIndex, bool &whetherEliminate,
                             std::vector<bool> &maskForEliminate_addMap, Graph &eliminationTrees_Update,
          ...
          for (LLint instance_j = 0; instance_j < sizeOfVariables[j]; instance_j++) {
            ...
            for (int k = 0; k < N; k++) {
              ...
              for (LLint instance_k = 0; instance_k < sizeOfVariables[k]; instance_k++) {
                ...
                sumIJK +=
                    ComputationTime_IJK(startTime_i, tasks[i], startTime_j, tasks[j], startTime_k, tasks[k]);
              }
            }
              ...
              // because one variable j can only depend on one single variable i;
              {
                ...
                // if (true)
                {
                  ...
                  if (tightEliminate == 1) {
                    ...
                    double distt = startTimeVector(index_j_overall, 0) - startTimeVector(index_i_overall, 0);

                    MappingDataStruct m{index_i_overall, distt};
                    mapIndex[index_j_overall] = m;
                  } else {
                  ...
                  boost::tie(e, inserted) =
                      add_edge(indexesBGL_Update[index_j_overall], indexesBGL_Update[index_i_overall],
  ...
  void addMappingFunction(VectorDynamic &resTemp, MAP_Index2Data &mapIndex, bool &whetherEliminate,
                          std::vector<bool> &maskForEliminate_addMap, Graph &eliminationTrees_Update,
                          indexVertexMap &indexesBGL_Update) {
    BeginTimer("addMap2");
        ...
        for (LLint j = i + 1; j < LLint(intervalVec.size()); j++) {
          ...
          for (LLint k = 0; k < LLint(intervalVec.size()); k++) {
            if (intervalVec[k].start >= endTime)
              break;
            sumIJK += ComputationTime_IJK(intervalVec[i], intervalVec[j], intervalVec[k]);
          }
          double distanceToBound_j_i =
              intervalVec[j].start + intervalVec[j].length - intervalVec[i].start - sumIJK;
          ...
          if ((distanceToBound_j_i < toleranceEliminator && distanceToBound_j_i >= 0)) {
            ...
            if (CheckNoConflictionTree(tree_i, tree_j, startTimeVector)) {
              ...
              boost::tie(e, inserted) = add_edge(indexesBGL_Update[index_j_overall],
                                                 indexesBGL_Update[index_i_overall], eliminationTrees_Update);
              // if (inserted)
              // {
              //     edge_name_map_t edgeMapCurr = get(edge_name, eliminationTrees_Update);
              //     edgeMapCurr[e] = mapIndex[index_j_overall].getDistance();
   ...
   */
  std::vector<Interval> CreateIntervalFromSTVSameOrder(std::vector<LLint> &tree1,
                                                       const VectorDynamic &startTimeVector) const {
    LLint n = tree1.size();
  ...
  MatrixDynamic DBFJacobian(boost::function<VectorDynamic(const VectorDynamic &)> h, const VectorDynamic &x,
                            double deltaOptimizer, int mOfJacobian) const {
    int n = x.rows();
  ...
  void textOld() {
    ...
    //                         for (LLint instance_k = 0; instance_k < sizeOfVariables[k]; instance_k++)
    //                         {
    //                             double startTime_k = ExtractVariable(startTimeVector, sizeOfVariables, k,
    //                             instance_k); sumIJK += ComputationTime_IJK(startTime_i, tasks[i],
    //                             startTime_j, tasks[j], startTime_k, tasks[k]);
    //                         }
    //                     }
    //                     double valueT = Barrier(startTime_j + tasks[j].executionTime - startTime_i - sumIJK
    //                     + 0); res(indexRes, 0) += valueT;
    //                 }
    //                 else
    //                 {
