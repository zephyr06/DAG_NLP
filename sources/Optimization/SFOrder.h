...
namespace OrderOptDAG_SPACE
{
  ...
  {
    JobIndexHelper() {}
    JobIndexHelper(const JobCEC &job, int offset) : job_within_hp(job), index_offset(offset) {}
    JobCEC job_within_hp;
    int index_offset; // used only in SFOrder
  };

  JobIndexHelper MapJobIndexToHpInfo(JobCEC job, const TaskSetInfoDerived &tasksInfo);
