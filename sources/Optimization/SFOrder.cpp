...
bool compareTimeInstance(const TimeInstance i1, const TimeInstance i2) {
    if (std::abs(i1.getTime() - i2.getTime()) >
        GlobalVariablesDAGOpt::LPTolerance / 10)
...
JobIndexHelper MapJobIndexToHpInfo(JobCEC job,
    ...
    int job_num_within_hp = tasksInfo.sizeOfVariables[job.taskId];
    if (job.jobId >= 0) {
        JobCEC jobWithinSingleHP{job.taskId, job.jobId % job_num_within_hp};
        return JobIndexHelper(jobWithinSingleHP, job.jobId / job_num_within_hp *
                                                     tasksInfo.length * 2);
    } else  // (job.jobId < 0)
    {
        if (job.jobId % job_num_within_hp == 0) {
            JobCEC jobWithinSingleHP{job.taskId, job.jobId % job_num_within_hp};
            return JobIndexHelper(
                jobWithinSingleHP,
                (job.jobId / job_num_within_hp) * tasksInfo.length * 2);
        } else {
            JobCEC jobWithinSingleHP{
                job.taskId, job.jobId % job_num_within_hp + job_num_within_hp};
            return JobIndexHelper(
                jobWithinSingleHP,
                (job.jobId / job_num_within_hp - 1) * tasksInfo.length * 2);
        }
    }
}
...
std::vector<TimeInstance> ExtractSubInstances(
    ...
    for (size_t i = minIndex; i < maxIndex; i++) {
        ...
        if (instCurr.type == 's')
            ...
            CoutError(
                "ExtractSubInstances: unrecognized type in TimeInstance!");
...
void SFOrder::InsertFinish(JobCEC job, LLint position) {
    RangeCheck(position, true);
    instanceOrder_.insert(instanceOrder_.begin() + position,
                          TimeInstance('f', job));
    // EstablishJobSFMap();
    whetherSFMapNeedUpdate = true;
}
...
void SFOrder::print() const {
    ...
    for (uint i = 0; i < instanceOrder_.size(); i++) {
        std::cout << instanceOrder_[i].job.taskId << ", "
                  << instanceOrder_[i].job.jobId << ", "
...
bool SFOrder::operator!=(const SFOrder &anotherJobOrder) const {
    return !(*this == anotherJobOrder);
}

LLint PredictInstanceIndexAfterRemove(LLint inst_index,
                                      LLint inst_to_remove_index) {
    if (inst_to_remove_index < inst_index)
        return inst_index - 1;
    else if (inst_to_remove_index > inst_index)
        return inst_index;
    else
        CoutError("Undefined behavior in PredictInstanceIndexAfterRemove!");
    return 0;
}

LLint PredictInstanceIndexAfterInsert(LLint inst_index,
                                      LLint inst_insert_index) {
    if (inst_insert_index <= inst_index)
        return inst_index + 1;
    else  // (inst_insert_index > inst_index)
        return inst_index;
}
...
LLint PredictInstanceIndexAfterRemoveInsertJob(LLint inst_index,
    ...
    inst_index = PredictInstanceIndexAfterRemove(inst_index, job_old_finish);
    inst_index = PredictInstanceIndexAfterRemove(inst_index, job_old_start);
    inst_index = PredictInstanceIndexAfterInsert(inst_index, job_new_start);
    inst_index = PredictInstanceIndexAfterInsert(inst_index, job_new_finish);
    return inst_index;
}
}  // namespace OrderOptDAG_SPACE
