...
public:
  ...
  ID_INSTANCE_PAIR popLeastFinishTime(const TaskSetInfoDerived &tasksInfo, const DAG_Model &dagTasks,
                                      double modifyPriorityBasedOnPrecedence = 0);
};
/**
...
void AddTasksToRunQueues(std::vector<RunQueue> &runQueues, const TaskSet &tasks,
                         ProcessorId2Index &processorId2Index, LLint timeNow);
...
void UpdateSTVAfterPopTask(RunQueue::ID_INSTANCE_PAIR &job, VectorDynamic &startTimeVector, LLint timeNow,
                           std::vector<LLint> &nextFree, const TaskSet &tasks, std::vector<bool> &busy,
                           int processorId, std::vector<LLint> &sizeOfVariables);

/**
 * @brief Warning! All the task sets must have int type values, otherwise it may generate inappropriate
 * initialization method; If you need to use double type, please use GlobalVariablesDAGOpt::timeScaleFactor to
 * transform it into int type with some acceptable accuracy.
 *
 ...
 */
VectorDynamic SimulateFixedPrioritySched(const DAG_Model &dagTasks, const TaskSetInfoDerived &tasksInfo,
                                         LLint timeInitial = 0);
