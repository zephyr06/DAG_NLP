...
class SFOrderLPOptimizer {
   public:
    SFOrderLPOptimizer(const DAG_Model &dagTasks, SFOrder &sfOrder,
                       int processorNum, std::string obj_type)
        : dagTasks_(dagTasks),
          sfOrder_(sfOrder),
          processorNum_(processorNum),
          obj_type_trait_(obj_type) {
        env_.end();
    ...
    inline void setOptimizedStartTimeVector(
        const VectorDynamic &optimizedStartTimeVector) {
        optimizedStartTimeVector_ = optimizedStartTimeVector;
    }
    inline void setOptimizedStartTimeVector() {
        optimizedStartTimeVector_ =
            GenerateVectorDynamic(tasksInfo_.variableDimension);
    }
