...
namespace OrderOptDAG_SPACE
{
                                              ...
                                              const int processorNum, std::vector<uint> &processorJobVec)
    {
        ...
        for (LLint i = 0; i < sfOrder.size(); i++)
        {
            ...
            if (orderInstance.type == 's')
            {
                ...
                for (int n = 0; n < processorNum; n++)
                {
                    if (!processorBusy[n])
                    {
                        ...
                        processorJobVec[IndexTran_Instance2Overall(orderInstance.job.taskId, orderInstance.job.jobId, tasksInfo.sizeOfVariables)] = n;
                        break;
                    }
                }
            ...
            {
                processorBusy[processorJobVec[IndexTran_Instance2Overall(orderInstance.job.taskId, orderInstance.job.jobId, tasksInfo.sizeOfVariables)]] = false;
            }
        }
        return true;
    }
