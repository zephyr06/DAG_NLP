...
class DAGScheduleOptimizer_Threshold : public DAGScheduleOptimizer<OrderScheduler, ObjectiveFunctionBase> {
public:
  ...
  DAGScheduleOptimizer_Threshold(const DAG_Model &dagInput, const ScheduleOptions &scheduleOptions,
                                 double timeLimits = GlobalVariablesDAGOpt::OPTIMIZE_TIME_LIMIT)
      : DAGScheduleOptimizer<OrderScheduler, ObjectiveFunctionBase>(dagInput, scheduleOptions, timeLimits) {
  ...
  bool CompareAndUpdateStatus(SFOrder &jobOrderCurrForFinish,
    ...
    auto simple_scheduler_startTimeVector =
        SimpleOrderScheduler::schedule(dagTasks, tasksInfo, scheduleOptions, jobOrderCurrForFinish,
                                       processorJobVec, ObjectiveFunctionBase::type_trait);
    double simple_scheduler_obj = ObjectiveFunctionBase::Evaluate(
        dagTasks, tasksInfo, simple_scheduler_startTimeVector, scheduleOptions);
    bool simple_scheduler_schedulable =
        ExamBasic_Feasibility(dagTasks, tasksInfo, simple_scheduler_startTimeVector, processorJobVec,
                              scheduleOptions.processorNum_);
    double simple_threshold = 0;
    if (simple_scheduler_schedulable) {
      LongestCAChain longestJobChains =
          LongestCAChain(dagTasks, tasksInfo, jobOrderBestFound, simple_scheduler_startTimeVector,
      ...
      for (auto &chain : dagTasks.chains_) {
        ...
        for (auto &longestJobChain : longestJobChains.longestChains_) {
          if (longestJobChain.size() == chain.size()) {
            bool match = true;
            for (int chainIdx = 0; chainIdx < static_cast<int>(chain.size()); chainIdx++) {
            ...
            if (match) {
              simple_threshold -=
                  GetStartTime(longestJobChain[0], simple_scheduler_startTimeVector, tasksInfo) -
    ...
    //         std::cout << "Threshold diff statistics:  Avg. " << accumulate(threshold_diff_vec_.begin(),
    //         threshold_diff_vec_.end(), 0) / threshold_diff_vec_.size()
    //             << " , total count: " << threshold_diff_vec_.size()
    //             << std::endl;
    //     }
    // }

    // if (simple_scheduler_schedulable && lp_obj < statusBestFound.objWeighted_ && simple_scheduler_obj -
    // simple_threshold > statusBestFound.objWeighted_) {
    ...
    return Base::WhetherMakeProgressAndUpdateBestFoundStatus(startTimeVector, processorJobVec, schedulable,
                                                             jobOrderCurrForFinish, statusBestFound,
                                                             jobOrderBestFound);
  }
...
template <typename OrderScheduler, typename ObjectiveFunctionBase>
ScheduleResult ScheduleDAGModel_Threshold(DAG_Model &dagTasks, const ScheduleOptions &scheduleOptions,
                                          double timeLimits = GlobalVariablesDAGOpt::OPTIMIZE_TIME_LIMIT) {
  CheckValidDAGTaskSetGivenObjType(dagTasks, ObjectiveFunctionBase::type_trait);
  DAGScheduleOptimizer_Threshold<OrderScheduler, ObjectiveFunctionBase> dagScheduleOptimizer(
      dagTasks, scheduleOptions, timeLimits);
