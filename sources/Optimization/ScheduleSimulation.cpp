...
RunQueue::ID_INSTANCE_PAIR RunQueue::popLeastFinishTime(const TaskSetInfoDerived &tasksInfo,
...
void UpdateSTVAfterPopTask(RunQueue::ID_INSTANCE_PAIR &job, VectorDynamic &startTimeVector, LLint timeNow,
                           ...
                           int processorId, const std::vector<LLint> &sizeOfVariables) {
  int id = job.first;
...
VectorDynamic ListSchedulingLFTPA(const DAG_Model &dagTasks, const TaskSetInfoDerived &tasksInfo,
  ...
  // std::vector<LLint> jobScheduled(jobOrder ? ((*jobOrder).size()) : 0, -1); // order is the same as
  // JobOrder's jobs
  EventPool eventPool;
  eventPool.Insert(0);
  ...
  for (LLint timeNow = timeInitial; timeNow < tasksInfo.hyperPeriod;) {
    ...
    for (int processorId = 0; processorId < processorNum; processorId++) {
      ...
      if (!busy[processorId] && (!runQueue.empty())) {
        ...
        UpdateSTVAfterPopTask(p, initial, timeNow, nextFree, tasks, busy, processorId,
                              tasksInfo.sizeOfVariables);
        eventPool.Insert(nextFree[processorId]);
        if (processorIdVec) {
          (*processorIdVec)[IndexTran_Instance2Overall(p.first, p.second, tasksInfo.sizeOfVariables)] =
              processorId;
        }
      }
    }
  ...
  for (auto &currentInstance : instanceOrder) {
    if (currentInstance.type == 'f') {
      if (scheduledFinishTime[GetJobUniqueId(currentInstance.job, tasksInfo)] < timeNow) {
        startTimeVector = GetInfeasibleStartTimeVector(tasksInfo);
        break;
      } else {
