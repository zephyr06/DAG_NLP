...
struct ScheduleResult {
    ...
    bool discardResult_ = false;  // if true, we'll discard this analysis result
                                  // when analyzing results later

    ScheduleResult() { obj_ = -1; }
    ScheduleResult(SFOrder sfOrder, VectorDynamic startTimeVector,
                   bool schedulable, double obj)
        : startTimeVector_(startTimeVector),
...
inline bool ExamDDL_Feasibility(
    const DAG_Model &dagTasks,
    const RegularTaskSystem::TaskSetInfoDerived &tasksInfo,
    const VectorDynamic &startTimeVector) {
...
ScheduleResult ScheduleDAGLS_LFT(
    ...
    VectorDynamic initialSTV = ListSchedulingLFTPA(
        dagTasks, tasksInfo, scheduleOptions.processorNum_, processorJobVec);
    ...
    if (ObjectiveFunctionBase::type_trait == "ReactionTimeObj" ||
        ...
        CoutError("Unknown obj type in ScheduleDAGLS_LFT");
    // if (ObjectiveFunctionBase::type_trait == "RTSS21ICObj") {
