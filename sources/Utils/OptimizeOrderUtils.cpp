...
void PrintResultAnalyzation(const ScheduleResult &scheduleResult,
                            const DAG_Model &dagTasks) {
    const TaskSet &tasks = dagTasks.tasks;
    RegularTaskSystem::TaskSetInfoDerived tasksInfo(tasks);
    std::cout << Color::blue;
    std::cout << "Schedulable after optimization? "
              << scheduleResult.schedulable_ << std::endl;
    std::cout << "Cause effect chains:" << std::endl;
    PrintChains(dagTasks.chains_);
    RTDA resAfterOpt = GetMaxRTDA(tasksInfo, dagTasks.chains_[0],
                                  scheduleResult.startTimeVector_);
    resAfterOpt.print();

    VectorDynamic sfError = OrderOptDAG_SPACE::ObtainSensorFusionError(
        dagTasks, tasksInfo, scheduleResult.startTimeVector_);
    std::cout << "SF Bound from schedule: " << sfError.maxCoeff() << std::endl;
    std::cout << "Max allowed SF Bound: "
              << GlobalVariablesDAGOpt::sensorFusionTolerance << std::endl;
...
bool CheckDDLConstraint(const DAG_Model &dagTasks,
    ...
    for (int i = 0; i < tasksInfo.N; i++) {
        for (int j = 0; j < int(tasksInfo.sizeOfVariables[i]); j++) {
            ...
            if (start < GetActivationTime(job, tasksInfo) ||
                start + GetExecutionTime(job, tasksInfo) >
                    GetDeadline(job, tasksInfo))
                return false;
        }
    }
    return true;
...
std::vector<std::vector<Interval>> ExtractJobsPerProcessor(
    const DAG_Model &dagTasks,
    const RegularTaskSystem::TaskSetInfoDerived &tasksInfo,
    const VectorDynamic &startTimeVector,
    ...
    for (uint i = 0; i < dagTasks.tasks.size(); i++) {
        for (uint j = 0; j < tasksInfo.sizeOfVariables[i]; j++) {
            JobCEC job(i, j);
            Interval v(GetStartTime(job, startTimeVector, tasksInfo),
                       tasksInfo.tasks[i].executionTime);
            if (processorJobVec[index] >= jobsPerProcessor.size()) {
                if (GlobalVariablesDAGOpt::debugMode)
                    CoutWarning(
                        "Wrong processorNum in ExtractJobsPerProcessor!");
...
double DBF_Error(const DAG_Model &dagTasks,
                 const RegularTaskSystem::TaskSetInfoDerived &tasksInfo,
                 const VectorDynamic &startTimeVector,
                 const std::vector<uint> &processorJobVec, int processorNum) {
    if (processorNum <= 0)
    ...
    std::vector<std::vector<Interval>> jobsPerProcessor =
        ExtractJobsPerProcessor(dagTasks, tasksInfo, startTimeVector,
...
bool ExamDBF_Feasibility(const DAG_Model &dagTasks,
                         const RegularTaskSystem::TaskSetInfoDerived &tasksInfo,
                         const VectorDynamic &startTimeVector,
...
bool ExamBasic_Feasibility(
    ...
    bool schedulable =
        ExamDDL_Feasibility(dagTasks, tasksInfo, startTimeVector) &&
        ExamDBF_Feasibility(dagTasks, tasksInfo, startTimeVector,
                            processorJobVec, processorNum);
    // EndTimer("ExamBasic_Feasibility");
    return schedulable;
}

bool ExamAll_Feasibility(const DAG_Model &dagTasks,
                         const RegularTaskSystem::TaskSetInfoDerived &tasksInfo,
                         const VectorDynamic &startTimeVector,
                         const std::vector<uint> &processorJobVec,
                         int processorNum) {
    if (!ExamBasic_Feasibility(dagTasks, tasksInfo, startTimeVector,
                               processorJobVec, processorNum))
        return false;
    ...
    for (auto chain : dagTasks.chains_) {
        std::vector<OrderOptDAG_SPACE::RTDA> rtdaVec =
            OrderOptDAG_SPACE::GetRTDAFromSingleJob(tasksInfo, chain,
                                                    startTimeVector);
...
//                                  const OptimizeSF::ScheduleOptions
//                                  &scheduleOptions, double sfBound, double
//                                  freshnessBound) {
//   TaskSet &tasks = dagTasks.tasks;
//   RegularTaskSystem::TaskSetInfoDerived tasksInfo(tasks);
//   std::vector<uint> processorJobVec;
//   VectorDynamic initialSTV =
//       ListSchedulingLFTPA(dagTasks, tasksInfo, scheduleOptions.processorNum_,
//       processorJobVec);

//   ScheduleResult res;
//   res.startTimeVector_ = initialSTV;
//   if (ObjectiveFunctionBase::type_trait == "RTDAExperimentObj") {
//     res.schedulable_ = ExamBasic_Feasibility(dagTasks, tasksInfo, initialSTV,
//     processorJobVec,
//                                              scheduleOptions.processorNum_);
//   } else if (ObjectiveFunctionBase::type_trait == "RTSS21ICObj") {
//     res.schedulable_ = ExamAll_Feasibility(dagTasks, tasksInfo, initialSTV,
//     processorJobVec,
//                                            scheduleOptions.processorNum_,
//                                            sfBound, freshnessBound);
//   }

//   res.obj_ = ObjectiveFunctionBase::TrueObj(dagTasks, tasksInfo, initialSTV,
//   scheduleOptions); return res;
// }
}  // namespace OrderOptDAG_SPACE