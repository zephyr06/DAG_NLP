...
OrderOptDAG_SPACE::ScheduleResult PerformSingleScheduling(DAG_Model &dagTasks,
  ...
  case InitialMethod:
    res = OrderOptDAG_SPACE::ScheduleDAGLS_LFT<ObjectiveFunctionBase>(dagTasks, scheduleOptions);
    break;
  case Verucchi20:
    res = ScheduleVerucchiRTDA(dagTasks, dagTasks.chains_, scheduleOptions.processorNum_,
  ...
  case TOM_IA:
    res = OrderOptDAG_SPACE::OptimizeSF::ScheduleDAGModel_IA<LPOrderScheduler, ObjectiveFunctionBase>(
        dagTasks, scheduleOptions);
    break;

  case TOM_Fast:
    res = OrderOptDAG_SPACE::OptimizeSF::ScheduleDAGModel<SimpleOrderScheduler, ObjectiveFunctionBase>(
        dagTasks, scheduleOptions);
    break;

  case TOM_Threshold:
    res = OrderOptDAG_SPACE::OptimizeSF::ScheduleDAGModel_Threshold<LPOrderScheduler, ObjectiveFunctionBase>(
        dagTasks, scheduleOptions);
    break;

  case TOM_RandomSearch:
    res =
        OrderOptDAG_SPACE::OptimizeSF::ScheduleDAGModel_RandomSearch<LPOrderScheduler, ObjectiveFunctionBase>(
            dagTasks, scheduleOptions);
    break;

  case TOM_Raw:
    res = OrderOptDAG_SPACE::OptimizeSF::ScheduleDAGModel_Raw<LPOrderScheduler, ObjectiveFunctionBase>(
        dagTasks, scheduleOptions);
    break;

  case TOM_FarReach:
    res = OrderOptDAG_SPACE::OptimizeSF::ScheduleDAGModel_FarReach<LPOrderSchedulerFlexJobOrder,
                                                                   ObjectiveFunctionBase>(dagTasks,
                                                                                          scheduleOptions);
    break;
...
struct BatchSettings {
  BatchSettings()
      : N(5), file_index_begin(0), file_index_end(10), folder_relative_path("TaskData/dagTasks/") {}
  BatchSettings(int N, int begin, int end, const std::string &folder_relative_path)
      : N(N), file_index_begin(begin), file_index_end(end), folder_relative_path(folder_relative_path) {}
...
BatchOptimizeOrder(std::vector<OrderOptDAG_SPACE::BASELINEMETHODS> &baselineMethods,
                   BatchSettings batchSettings = BatchSettings()) {

  srand(time(0));

  std::string dataSetFolder = GlobalVariablesDAGOpt::PROJECT_PATH + batchSettings.folder_relative_path;
  ...
  for (int fileIndex = batchSettings.file_index_begin; fileIndex < batchSettings.file_index_end;
       fileIndex++) {
    ...
    for (auto batchTestMethod : baselineMethods) {
      ...
      std::cout << Color::blue << "Method: " << BaselineMethodNames[batchTestMethod] << Color::def
                << std::endl;
      if (VerifyResFileExist(pathDataset, file, batchTestMethod, ObjectiveFunctionBase::type_trait)) {
        res = ReadFromResultFile(pathDataset, file, batchTestMethod, ObjectiveFunctionBase::type_trait);
      } else {
        auto start = std::chrono::high_resolution_clock::now();
        res = PerformSingleScheduling<ObjectiveFunctionBase>(dagTasks, scheduleOptions, batchTestMethod);
      ...
      // optimal
      if (res.timeTaken_ > GlobalVariablesDAGOpt::kGlobalOptimizationTimeLimit - 10 &&
      ...
      WriteToResultFile(pathDataset, file, res, batchTestMethod, ObjectiveFunctionBase::type_trait);
  ...
  // result analysis
  results_man.PrintWorseCase(BASELINEMETHODS::InitialMethod, BASELINEMETHODS::TOM);
